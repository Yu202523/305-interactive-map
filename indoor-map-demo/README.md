...
- 地圖點擊後，系統會先把滑鼠位置換算成 SVG 座標
- 系統會先檢查該座標是否落在任一個 obstacle 矩形內
- 如果沒有落在障礙物上，就直接使用該座標
- 如果落在障礙物上，系統會改找障礙物四周最近的可用安全點
- 安全點會再限制在房間邊界內，避免跑出地圖外框
- 如果找不到可用安全點，系統會提示使用者重新選點
